...
try:
    ...
    print("Vui lòng đảm bảo file giao diện này nằm cùng thư mục với congThuc.py và quanLiDoThi.py")
...
def solve():
    try:
        ...
        try:
            ...
            if not has_trace:
                output.insert(tk.END, "(Chưa có dữ liệu suy luận. Hoặc thiếu dữ kiện đầu vào, hoặc Rules chưa hoạt động)\n")
        except AttributeError:
            output.insert(tk.END, "Lỗi: Hàm lay_do_thi() không tồn tại trong QuanLiDoThi.\n")

    except ValueError:
        # Lỗi khi người dùng nhập chữ cái thay vì số
        output.delete(1.0, tk.END)
        output.insert(tk.END, "❌ Lỗi: Dữ liệu nhập vào phải là số hợp lệ!")
        messagebox.showwarning("Sai định dạng", "Vui lòng chỉ nhập số vào các ô dữ kiện.")
        
    except Exception as e:
        # BẮT VÀ HIỂN THỊ LỖI THẬT SỰ TỪ THUẬT TOÁN ĐỂ DEBUG
        output.delete(1.0, tk.END)
        error_msg = traceback.format_exc()
        output.insert(tk.END, f"❌ LỖI HỆ THỐNG / LỖI LOGIC TỪ FILE THUẬT TOÁN:\n\n{error_msg}")
...
# ===== TIÊU ĐỀ =====
tk.Label(window, text="GIẢI BÀI TOÁN TAM GIÁC (MẠNG NGỮ NGHĨA)", font=("Arial", 16, "bold"), fg="#2c3e50").pack(pady=10)

# ===== KHU VỰC NHẬP LIỆU =====
frame_input = tk.LabelFrame(window, text="Nhập dữ kiện ban đầu (Để trống nếu không biết)", padx=10, pady=10)
...
for i, label in enumerate(labels):
    tk.Label(frame_input, text=f"{label} = ", font=("Arial", 11, "bold")).grid(row=i//3, column=(i%3)*2, padx=5, pady=10, sticky="e")
...
tk.Button(frame_btn, text="🚀 GIẢI BÀI TOÁN", width=15, command=solve, bg="#27ae60", fg="white", font=("Arial", 11, "bold")).grid(row=0, column=0, padx=10, ipady=3)
tk.Button(frame_btn, text="🗑️ Xóa Trắng", width=12, command=clear, bg="#e74c3c", fg="white", font=("Arial", 11, "bold")).grid(row=0, column=1, ipady=3)
...
output = tk.Text(frame_output, yscrollcommand=scrollbar_out.set, font=("Consolas", 11), bg="#fdfdfd")
output.pack(fill="both", expand=True)
scrollbar_out.config(command=output.yview)

# ===== KHU VỰC HIỂN THỊ 20 CÔNG THỨC =====
frame_formula = tk.LabelFrame(window, text="Danh sách tập luật (20 Công thức)", padx=10, pady=10)
...
formula_text = tk.Text(frame_formula, height=8, yscrollcommand=scrollbar_form.set, font=("Consolas", 10), bg="#f4f6f7")
